...
const ripotiMenejaRequestRouter = express.Router();
const { isAuth, formatDate } = require("../../utils");
const sharedModel = require("../../models/sharedModel");


// List of
ripotiMenejaRequestRouter.get("/ripoti-meneja-shule", isAuth, (req, res) => {
  const user = req.user;
      ...
      sharedModel.getRegistrationStructures((structures) => {
        sharedModel.getRegions(user , (regions) => {
            ...
            end_date = formatDate(date_range.split("to")[1], "YYYY-MM-DD");
              ...
              FROM (
                SELECT
                  ...
                  TRIM(CONCAT_WS(' ', m.manager_first_name, m.manager_middle_name, m.manager_last_name)) AS manager_name,
                ...
                FROM applications a
                  JOIN managers m ON m.tracking_number = a.tracking_number
                  JOIN establishing_schools e ON e.id = m.establishing_school_id
          ...
          const sqlRows = `SELECT tracking_number AS tracking_number ,manager_name, school_name , category, structure, 
	                     ...
	                     LIMIT ?, ?`;
	          const sqlCount = `SELECT COUNT(*) AS num_rows ${managersFrom}`;
...
module.exports = ripotiMenejaRequestRouter;
