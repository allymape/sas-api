...
const ripotiUhamishoChangeRequestRouter = express.Router();
const { isAuth, formatDate } = require("../../utils");
const sharedModel = require("../../models/sharedModel");


// List of
ripotiUhamishoChangeRequestRouter.get("/ripoti-hamisha-shule", isAuth, (req, res) => {
	  const user = req.user;
	  ...
	  sharedModel.getSchoolCategories((categories) => {
	    sharedModel.getSchoolOwnerships((ownerships) => {
            ...
            end_date = formatDate(date_range.split("to")[1], "YYYY-MM-DD");
              ...
              FROM (
                SELECT
                  ...
                  orr.RegionName AS old_region,
                  od.LgaName AS old_district,
                  ow.WardName AS old_ward,
                  ost.StreetName AS old_street,
                ...
                FROM applications a
                  ...
                  JOIN wards ow ON ow.WardCode = fsi.ward_id
                  JOIN streets ost ON ost.StreetCode = fsi.village_id AND ost.WardCode = ow.WardCode
                  JOIN districts od ON od.LgaCode = ow.LgaCode
                  JOIN regions orr ON orr.RegionCode = od.RegionCode
                ...
                WHERE a.application_category_id = 10
              ) transfer_change_view
          ...
          const sqlRows = `SELECT tracking_number AS tracking_number , school_name , category, structure, 
                            registry , region, district , ward , street , old_region, old_district, old_ward, old_street , is_approved AS status, 
	                     ...
	                     LIMIT ?, ?`;
	          const sqlCount = `SELECT COUNT(*) AS num_rows ${transfersFrom}`;
...
module.exports = ripotiUhamishoChangeRequestRouter;
