...
const ripotiKufutaChangeRequestRouter = express.Router();
const { isAuth, formatDate } = require("../../utils");
const sharedModel = require("../../models/sharedModel");


// List of
ripotiKufutaChangeRequestRouter.get("/ripoti-kufuta-shule", isAuth, (req, res) => {
	  const user = req.user;
	    ...
	    sharedModel.getSchoolOwnerships((ownerships) => {
          ...
          if (date_range) {
            ...
            end_date = formatDate(date_range.split("to")[1], "YYYY-MM-DD");
              ...
              FROM (
                ...
                WHERE a.application_category_id = 11
                  AND sr.reg_status IN (0,1)
              ) deregistration_change_view
          ...
          const sqlRows = `SELECT tracking_number AS tracking_number ,school_name , category, structure, 
                            registry , region, district , ward , street , is_approved AS status, 
	                            ...
	                            END AS approved,
	                            approved_at
	                     ${deregistrationFrom} ORDER BY school_name DESC
	                     LIMIT ?, ?`;
	          const sqlCount = `SELECT COUNT(*) AS num_rows ${deregistrationFrom}`;
          // console.log(sqlRows)
...
module.exports = ripotiKufutaChangeRequestRouter;
