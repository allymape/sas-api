...
const ripotiKuanzishaRequestRouter = express.Router();
const { isAuth, formatDate } = require("../../utils");
const sharedModel = require("../../models/sharedModel");


// List of
ripotiKuanzishaRequestRouter.get("/ripoti-kuanzisha-shule", isAuth, (req, res) => {
  const user = req.user;
      ...
      sharedModel.getRegistrationStructures((structures) => {
        sharedModel.getRegions(user , (regions) => {
          ...
          if (date_range) {
            ...
            end_date = formatDate(date_range.split("to")[1], "YYYY-MM-DD");
              ...
              FROM (
                ...
                WHERE a.application_category_id = 1
              ) established_schools_view
          ...
          const sqlRows = `SELECT tracking_number AS tracking_number , school_name , category, structure, 
                            ...
                            END AS approved,
                            approved_at
	                     ${establishedSchoolsFrom} 
	                     ORDER BY school_name DESC
	                     LIMIT ?, ?`;
	          const sqlCount = `SELECT COUNT(*) AS num_rows ${establishedSchoolsFrom}`;
...
module.exports = ripotiKuanzishaRequestRouter;
