            ...
            end_date = formatDate(date_range.split("to")[1], "YYYY-MM-DD");
              ...
              FROM (
                SELECT
                  ...
                  LCASE(TRIM(CONCAT_WS(' ', fm.manager_first_name, fm.manager_middle_name, fm.manager_last_name))) AS old_manager_name,
                  LCASE(TRIM(CONCAT_WS(' ', m.manager_first_name, m.manager_middle_name, m.manager_last_name))) AS manager_name,
                ...
                WHERE a.application_category_id = 8
              ) managers_change_view
	          ...
	          const sqlRows = `SELECT tracking_number AS tracking_number , old_manager_name, manager_name, school_name , category, structure, 
	                     ...
	                     LIMIT ?, ?`;
	          const sqlCount = `SELECT COUNT(*) AS num_rows ${managersChangeFrom}`;
              ...
              return res.send({
                ...
                structures,
	                ...
	                message: error
	                  ...
	                  : "Ripoti ya kubadili meneja wa shule.",
	              });
	            },
	            [offset, per_page]
	          );
