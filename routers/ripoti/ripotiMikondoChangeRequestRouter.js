...
const ripotiMikondoChangeRequestRouter = express.Router();
const { isAuth, formatDate } = require("../../utils");
const sharedModel = require("../../models/sharedModel");
// List of
ripotiMikondoChangeRequestRouter.get("/ripoti-badili-mikondo", isAuth, (req, res) => {
	  const user = req.user;
	  ...
	  sharedModel.getSchoolCategories((categories) => {
	    sharedModel.getSchoolOwnerships((ownerships) => {
          ...
          if (date_range) {
            ...
            end_date = formatDate(date_range.split("to")[1], "YYYY-MM-DD");
              ...
              FROM (
                ...
                WHERE a.application_category_id = 5
              ) streams_change_view
          ...
          const sqlRows = `SELECT tracking_number AS tracking_number ,school_name , stream, old_stream , category, structure, 
	                     ...
	                     LIMIT ?, ?`;
	          const sqlCount = `SELECT COUNT(*) AS num_rows ${streamsFrom}`;
...
module.exports = ripotiMikondoChangeRequestRouter;
